...
<p>The SIR model is a very powerful tool (no doubt), but it was developed a century ago when scientific paradigms were different. Looking today at SIR with a critical eye immediately appears that it suffers from some approximations that limit its ability to represent reality.</p>
...
<p>The model allows parametrizing most of the key factors affecting the spread of the disease. On the one hand, that makes the model very powerful, being able to represent many possible dynamics. On the other hand, all this versatility can be disorienting.</p>
...
<p>If '<code>iterations = 1</code>' the distribution is homogeneous (linear), the same distribution when a single die is rolled.<br>
...
...<br>
When infinite dies are rolled, the distribution of the result is a Gaussian.</p>
...
<p>The <strong>second button</strong> (the camera) take snapshots of the application: the whole form or only the diagram.</p>
